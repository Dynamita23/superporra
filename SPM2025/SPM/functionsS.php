...
function conexion($bd_config){
	try {
	$conexion = new PDO('mysql:host=RUTA;dbname='.$bd_config['basedatos'], $bd_config['usuario'], $bd_config['pass']);
...
function obtener_post_por_id($conexion, $id){
	$resultado = $conexion->query("SELECT * FROM spm2021 WHERE Numero = $id LIMIT 1");
...
function obtener_post($post_por_pagina, $conexion){
	...
	$sentencia = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM spm2021 LIMIT 49");
...
function obtener_semanal($conexion){
	$resultado = $conexion->query("SELECT * FROM spm2021 WHERE M1 +1 LIMIT 49");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}
