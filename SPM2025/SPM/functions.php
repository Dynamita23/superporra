...
function obtener_post($post_por_pagina, $conexion){
	...
	$sentencia = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM spm2021 LIMIT {$inicio}, {$post_por_pagina}");
...
function obtener_semanal($conexion){
	$resultado = $conexion->query("SELECT * FROM spm2021 WHERE $sem LIMIT 49");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}
